...
def test_build_items_ladder_absent_streak_is_none_not_zero() -> None:
    ...
    zero = new_stock_service._build_items(
        [{"symbol": "601091", "name": "C沈鼓", "exchange": "Shanghai_Stocks"}], {"601091": 0}
    )
    assert zero[0]["streak"] == 0 and zero[0]["is_lu"] is False, "表内 0 是合法值，与缺失不同"
...
async def test_get_new_stock_board_merges_ladder_stats_and_membership(
    ...
    _patch_board_reads(
        ...
        enriched=[
            _enriched("601091", "C沈鼓", latest_price=20.8, change_percent=20.0),
            _enriched("688837", "C信诺维"),  # 无行情
        ],
        snap={
...
async def test_new_stocks_contract_matches_dev_db() -> None:
    ...
    # 未开板（上市以来每一行都涨停）⇒ 今天也在涨停 → 必然在梯队里。反例只可能来自今日停牌/退市
    # 的票，BK0501 全为 ≤1 年新股、当日全有行情，故该不变量应当成立（实测见 task-10-report.md）。
    assert kpis["unbroken_count"] <= kpis["limit_up_count"], (
        "未开板家数不可能超过当日涨停家数（同一 BK0501 数据上实测成立）"
    )
