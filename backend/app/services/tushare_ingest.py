...
class TuShareIngestService:
    ...
    async def ingest_stock_universe(
        ...
        *,
        ...
        for row in df_basic.to_dict("records"):
            ...
            detail: dict[str, Any] = {
                k: _to_builtin(v) for k, v in row.items() if k not in _tushare_direct_fields
            }
            ...
            if company_info:
                detail["company"] = {k: _to_builtin(v) for k, v in company_info.items()}
            ...
            if company_info:
                raw_record["__company__"] = {k: _to_builtin(v) for k, v in company_info.items()}
    ...
    async def ingest_daily_basic_for_stock(
        ...
        *,
        ...
        batch_size = 500
        for i in range(0, len(records), batch_size):
            upserted += await daily_basic_repo.upsert_daily_basics(db, records[i : i + batch_size])
