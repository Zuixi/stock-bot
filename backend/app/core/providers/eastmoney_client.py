...
_PAGE_SIZE = 100
# 翻页死循环护栏：total 撒谎 + 服务端永远返回同一页时，最多翻至此页数就收手。
# 20 → 60（2026-09-18）：最大板 BK0596 融资融券 member_total=3870，旧上限 20×100=2000 拿不全，
# T4 完整性门禁据此判 partial 并跳过 → 该板 0 成分，读路径 inner join 成员使其在任何概念视图
# 里都不可见。60×100=6000 覆盖最大板；小板仍靠空页收敛，不受此上限影响。
_MAX_PAGES = 60
