...
| 概念板块成分 | 板块列表 + 成分股 | 每日 18:20 | 东财 | 免费 |
...
统计带（TV 风）：**10 已上线数据域 · 5,500+ 标的 · 4 级数据权威分级 · 已上线数据全部免费**。

> 上表共 11 行（北向为「规划中」不计入统计带），与 `frontend/src/features/market/components/DataCoverageMatrix.tsx` 的 `COVERAGE_ROWS` 逐行同源；统计带的「已上线数据域」由 `STATS` 从行数推导。

> 北向资金标记「规划中」：`northbound_daily` 实测全表 0 行（非仅滞后），首页已整卡移除，
> 公开文案不得把规划中的域计入「已上线」统计，也不得标注来源（无源可溯即不署名）。
