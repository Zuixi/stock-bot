...
### UI 呈现契约（2026-09-15 重设计，与前端组件对齐）

**比值字段的百分比换算是 UI 职责**：`broken_rate` / `promo_1to2` / `promo_2to3` 后端是 0-1 比值，UI 必须 ×100 渲染（`0.4062 → 40.62%`）；直接 `toFixed(2)%` 会显示成 `0.41%`（2026-09-15 前的实际线上 bug）。`yzt_avg_pct` / `yzt_avg_open_premium` 本身已是百分数口径，原值直显。

**情绪温度计**（`SentimentThermometer`，替代旧 `SentimentHeader` datarow 版式）：

- 三大计数（涨停/炸板/跌停）大数字瓦片 + 多空力量条（家数占比，零值段隐藏）；
- 二级瓦片：炸板率 / 昨日涨停均值（注 `n=yzt_n`）/ 今开溢价均值 / 1进2 / 2进3（`noisy` 时数值弱化并注 `n=N，样本小`）/ 最高板；
- 环比 chip 与涨停/跌停趋势线来自 `GET /sentiment/calendar`：取 `trade_date < as_of` 的最近一点做环比，无前值只藏 chip 不造假；趋势序列在日历滞后于 `as_of` 时以实时 KPI 补末点，保证线尾与主卡一致；`<2` 点显示"累积中"占位。日历查询失败不阻断主数据。

**连板梯队**（`LimitUpLadder` 分档列版式）：

- 每档一卡：板高热度色阶（涨色 alpha 随板高加深，streak≥4 归最高档）+ 家数占比条 + 家数 chip；卡顶汇总 pill（空间板/连板/首板家数）是对已下发 echelons 的呈现级聚合，不新造口径；
- 档内按 `seal_time` 升序（越早封越强，null 排最后）；个股行可点跳 `/stock/:symbol`；
- 徽标披露：`N天M板` 仅在 `boards_in_window ≠ streak` 时显示；`缺少 N 个交易日`（停牌）恒显；`炸板 N 次` 仅 `>0` 时显示（0/null 不占位，封板质量由封单额表达）；封单额仅 Web 增强命中时显示；
- 首板档默认展示 12 家，超出折叠并提供"展开全部"（`sentiment-ladder` 根类名与上述文案是 e2e 契约，`e2e/limitUpSentiment.spec.ts`）。

**申万三级最高板**（`SwL3LimitUpBoard` 热度榜版式，2026-09-15 重设计替代 Segmented+Table）：

- 一级行业过滤用 CheckableTag（可换行 + 家数徽标）——block Segmented 塞 21 个 L1 会把标签截成单字（不可用）；「仅看 ≥2 板」开关缓解 38 行全量扫读；
- 行内板高渲染为 4 格热度色阶（与梯队同语义，streak≥4 封顶）+ 家数占比条；排序 板高 desc → 家数 desc → l3Code（确定性）；
- `.sector-limit-up` 根类与行点击跳龙头个股为 e2e 契约。

**「昨日涨停今日表现」的 as_of 口径（防误读，2026-09-15）**：卡头「数据截至」必须用**表现日** `as_of`（如 9/14）；涨停日样本 `as_of_prev`（如 9/11）在卡内首行显式标注（"统计 9月11日 涨停股在 9月14日 的表现 · 行情 T+1 回补"）。只标 `as_of_prev` 会被用户误读为"数据落后一个交易日"——该卡最新表现日比梯队卡看起来"旧"是双日口径使然，不是数据陈旧；当日表现等 T+1 行情回补后由下一次计算自然生成。
