...
- 审计型状态持久化应按语义区分冲突策略：事实事件用命名唯一约束配合 `ON CONFLICT DO NOTHING` 保证不可变，当前质量与到期评价用同一约束配合幂等 upsert，并让 repository/service 只 flush SQL、不越权 commit。

- 周期信号应在纯规则引擎调用前完成与引擎同源同频的质量门控，并冻结事件当时的指标期次、来源与验证规则后按目标日后的首个行业指标观测回评，避免数据缺失、来源漂移或 ETF 表现污染行业周期结论。

- 不可变状态转换若允许同日多次变化，应以单调 event_sequence 建模而非把状态纳入唯一键；验证快照必须同时冻结 source/freq，百分比边界使用 Decimal 字符串构造并将审计 JSON 数值安全序列化。

- 审计事件的同日序列分配必须在稳定键的事务 advisory lock 内重读后分配，已发布迁移不可原地改写；验证 provenance 应冻结实际选中行的 source 与 freq，而不是 registry 当前声明。

- 审计型聚合接口应只投影用户证据字段、把准确率的最小样本门槛放在后端统一执行，并严格在数据库提交成功后再失效缓存，避免泄露内部快照、客户端口径漂移和旧事务值回填缓存。
- 列表展示上限不得隐式限定统计摘要口径：摘要应走独立全量聚合查询；事务提交后的缓存失效属于可恢复副作用，需捕获广义异常并告警而不能把已成功持久化的任务改判失败，Pydantic 集合默认值统一使用 `Field(default_factory=...)`。
- 审计型 Dashboard 前端应在单一 mapper 中完成全部 snake_case→camelCase 转换并按契约保留后端摘要，组件只负责空值降级与状态展示，禁止客户端重新推导准确率等业务口径。
- 时间线事件必须透传后端稳定序列并以 `(event_date, event_sequence, horizon)` 构造 React/E2E 唯一标识；当前态为空时只降级当前卡片，不应连带隐藏独立的历史事件与聚合摘要，Tooltip 触发器应使用可聚焦原生控件覆盖键盘访问。

- Worker 返回值在跨越消息处理与 JSONB 持久化边界前必须统一转换为 JSON 兼容结构，尤其要覆盖嵌套 `date`/`datetime`/UUID/Decimal，并用真实 `json.dumps` 回归测试锁定该契约。
- 真实栈 E2E 断言必须遵循质量门禁与数据可用性契约：门禁失败时验证明确空态而非强求信号，外部基线数据缺失时应显式跳过依赖数据值的断言但仍验证控件与空态。
- 行业级数据质量评估必须按 coverage_scope 过滤评估对象：公司级指标（stock_id>0）在行业级查询中永远不可见，混入评估会产生永久性"缺失"幻影并在 UI 上挤掉真实问题；registry 声明与跳过谓词必须成对落地，否则过滤规则是空操作。
