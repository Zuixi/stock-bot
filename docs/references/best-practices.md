...
- 同一仓库被 Windows Git 与 WSL Git 双端检出时，`core.autocrlf` 必须固定到**仓库级**配置（两端共享 `.git/config`）：只在 Windows 侧设全局值，WSL 侧就会把「index 存 LF / 工作区是 CRLF」判成 380 个文件整体改写——`git status` 彻底失效、真实改动被淹没，行尾噪声还会在 rebase 时制造假冲突（本次 2 处真实改动被伪装成 380 个文件、+76296/-76278）。遇到「改动行数 ≈ 文件总行数」的假 diff，先用 `git diff --ignore-cr-at-eol` 或 `git -c core.autocrlf=true status` 对账确认是行尾噪声再动手，别急着重灌文件。
