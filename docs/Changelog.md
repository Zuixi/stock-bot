## 2026-09-21 - 发布链路打通：CD 补 forward-auth 镜像 + 裸数字 tag 触发 + 服务器 prod override 与数据迁移文档
- **问题**：已发布镜像这条路此前根本走不通——① `.github/workflows/cd.yml` 只发 3 个镜像，`forward-auth` 缺失，registry 部署起不来 `forward_auth`；② 触发条件只有 `tags: ["v*"]`，用户要推的裸 tag `0.0.1` **永远不触发 CD**；③ `docker-compose.yml` 每个应用服务都硬编码 `image: stock-bot-*:local` + `build:`（`frontend` 更是只有 `build:`），服务器没有 override 就必然本地构建；④ 服务器侧部署/数据迁移文档完全缺失
- **改动**：① `cd.yml` 新增 `IMAGE_NAME_FORWARD_AUTH` env + metadata + build-push 三个步骤（与其余三个同形，平台/registry/缓存不动），共 **4** 个镜像；② tag filters 改为 `["v*", "0.*"]` 并注明裸 tag 与 `v` 前缀都会产出 `0.0.1` + `0.0` + `<short-sha>`；③ 新增 `docker-compose.prod.yml`：8 个应用服务（`migrate`/`migrate-auth`/`auth-service`/`forward-auth`/`api`/`worker`/`scheduler`/`frontend`）显式 `ghcr.io/zuixi/stock-bot/{backend,auth-service,forward-auth,frontend}:${IMAGE_TAG:?…}` + `pull_policy: always`，基建/卷/网络/healthcheck/env 一律不动；④ `docs/build.md` 新增「服务器部署（发布镜像 + 数据迁移）」：发布流程（含 GHCR 凭据与 SWR 转推兜底）、`pull` + `up -d --no-build`、实测数据迁移（全量 501 MB/45 s，精简 264 MB/28 s，恢复 71 s，对账 6 张表 + `alembic_version=2614ed9a9ab4`）、`data_init` 开机回填 footgun 与两个安全做法、以及「不要复制 `docker-compose.override.yml`」的卷名陷阱
