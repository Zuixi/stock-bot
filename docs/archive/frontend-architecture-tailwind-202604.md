...
> **已归档（2026-09-24）**：本文描述 **2026-04 的 Tailwind CSS v4 + shadcn/ui 方案**，与当前实现（**Ant Design 5 + feature-sliced**）不一致，仅作历史参考。
>
> 当前态请读：[`../architecture/frontend/ARCHITECTURE.md`](../architecture/frontend/ARCHITECTURE.md)（架构与目录）· [`../frontend-ux-spec.md`](../frontend-ux-spec.md)（交互规格）· [`../authority.md`](../authority.md)（权威矩阵）。
>
> 保留原因：记录了 shadcn/ui 时代的分层与数据层设计意图与早期测试策略——属历史层，不追改。
