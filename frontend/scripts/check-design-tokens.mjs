...
const MODES = [
  { label: "fallback", marker: ":root {", tsMode: "light" },
  { label: "light", marker: '[data-theme="light"]', tsMode: "light" },
  { label: "dark", marker: '[data-theme="dark"]', tsMode: "dark" },
];

let failed = 0;
for (const { label, marker, tsMode } of MODES) {
  ...
  for (const name of ["up", "down"]) {
    ...
    const ok = ratio >= 4.5;
    console.log(
      `${ok ? "PASS" : "FAIL"} ${label} --${name} ${hex} on ${bg}: ${ratio.toFixed(2)}:1`
    );
    if (!ok) failed += 1;
